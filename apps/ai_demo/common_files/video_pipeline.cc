...
{
    const char *camera_csi = getenv("DSHANPI_CAMERA_CSI");
    csi_num_ = camera_csi != nullptr && atoi(camera_csi) == 2 ? 2 : 0;
    sensor_width_ = csi_num_ == 2 ? 1280 : 1920;
    sensor_height_ = csi_num_ == 2 ? 960 : 1080;
    // ------------------------ 显示接口类型选择 ------------------------
...
int PipeLine::Create()
{
    ...
    probe_cfg.width   = sensor_width_;
    probe_cfg.height  = sensor_height_;
    probe_cfg.fps     = 30;
    ...
    dev_attr.acq_win.width   = sensor_width_;
    dev_attr.acq_win.height  = sensor_height_;
