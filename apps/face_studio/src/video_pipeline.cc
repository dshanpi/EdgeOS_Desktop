...
namespace {
void draw_inference_fps(void *osd_data)
{
    ...
    cv::Mat osd(OSD_HEIGHT, OSD_WIDTH, CV_8UC4, osd_data);
    int left = OSD_WIDTH - 116;
    int top = OSD_HEIGHT - 46;
    ios_ui::rounded_rect(osd, cv::Rect(left, top, 108, 36),
                         ios_ui::glass(), cv::FILLED);
    cv::putText(osd, label, cv::Point(left + 8, top + 25),
                cv::FONT_HERSHEY_SIMPLEX, 0.55,
                ios_ui::white(), 2, cv::LINE_AA);
}
}
...
/* 构造函数：初始化管线各模块的默认配置 */
CameraManager::CameraManager(int debug_mode, int csi_num)
{
    csi_num_ = csi_num == 0 ? 0 : 2;
    // ------------------------ 显示接口类型选择 ------------------------
    ...
    // VICAP → capture 通道（保持固定，算法切换不重配管线）
    vicap_chn_to_capture = VICAP_CHN_ID_2;
...
int CameraManager::Create()
{
    ...
    /*
     * Match the LVGL Camera application's landscape orientation. The ST7701
     * panel is native 480x640 and the desktop configures its 640x480 camera
     * layer with a 270-degree GDMA rotation.
     */
    vi_vo_attr.func            = DISPLAY_MODE
                                     ? GDMA_ROTATE_DEGREE_270
                                     : GDMA_ROTATE_DEGREE_0;
    ...
    printf("VICAP to VO: layer=%d configured for %ux%u NV12, rotation=%d\n",
           vi_vo_id, DISPLAY_WIDTH, DISPLAY_HEIGHT,
           DISPLAY_MODE ? 270 : 0);
    ...
    if(USE_OSD == 1){
        ...
        osd_vo_attr.func            = DISPLAY_MODE
                                          ? GDMA_ROTATE_DEGREE_90
                                          : GDMA_ROTATE_DEGREE_0;
        ...
        printf("OSD to VO: layer=%d configured for %ux%u BGRA8888, rotation=%d\n",
    ...
    /* The selected Sensor is immutable for the lifetime of this Session. */
    probe_cfg.csi_num = csi_num_;
    probe_cfg.width   = csi_num_ == 0 ? 1920 : 1280;
    probe_cfg.height  = csi_num_ == 0 ? 1080 : 960;
    probe_cfg.fps     = 30;
    ...
    /*
     * CSI0 needs the rear-module 180-degree correction.  CSI2 is mounted in
     * the opposite vertical orientation and only needs a horizontal
     * front-camera mirror.  Apply this before both display and inference so
     * boxes, landmarks and labels remain in one coordinate system.
     */
    dev_attr.mirror = csi_num_ == 0 ? VICAP_MIRROR_BOTH
                                    : VICAP_MIRROR_HOR;
    ...
    // 通道 2：固定 NV12 capture 输出，供后续拍照/录像插件使用
    k_vicap_chn_attr chn2_attr;
    memset(&chn2_attr, 0, sizeof(k_vicap_chn_attr));
    chn2_attr.out_win.width  = DISPLAY_WIDTH;
    chn2_attr.out_win.height = DISPLAY_HEIGHT;
    chn2_attr.chn_enable     = K_TRUE;
    chn2_attr.pix_format     = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
    chn2_attr.buffer_num     = 4;
    chn2_attr.buffer_size    =
        VICAP_ALIGN_UP(DISPLAY_WIDTH * DISPLAY_HEIGHT * 3 / 2,
                       VICAP_ALIGN_1K);
    chn2_attr.buffer_pool_id = VB_INVALID_POOLID;
    ret = kd_mpi_vicap_set_chn_attr(vicap_dev, vicap_chn_to_capture,
                                    chn2_attr);
    if (ret) {
        printf("configure fixed capture channel failed, ret=%d\n", ret);
        return ret;
    }
    printf("[camera-manager] fixed channels: preview=0, ai=1, capture=2\n");
    ...
    if (ret) {
        printf("kd_mpi_vicap_init failed, ret=%d.\n", ret);
        return ret;
    }
    ...
    if (ret) {
        printf("kd_mpi_vicap_start_stream failed, ret=%d.\n", ret);
...
int CameraManager::GetFrame(DumpRes &dump_res){
    ...
    if (dump_res.virt_addr == 0 || dump_res.phy_addr == 0) {
        printf("camera manager rejected an invalid AI frame mapping\n");
...
/* 释放当前 dump 帧 */
int CameraManager::ReleaseFrame(DumpRes &dump_res){
...
int CameraManager::InsertFrame(void* osd_data, bool show_metrics){
    ScopedTiming st("PipeLine::InsertFrame", debug_mode_);
    int ret=0;

    if (show_metrics) {
        draw_inference_fps(osd_data);
    }
