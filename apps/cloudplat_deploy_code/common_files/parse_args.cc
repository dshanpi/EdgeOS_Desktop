...
void parse_args(std::string config, config_args& args, int debug_mode)
{
    ...
    else
    {
        ...
        if (reader.parse(srcFile, root))
        {
            ...
            /* The cloud package can retain an absolute path from the
             * training host. The LVGL deployment workflow lets users copy
             * only the model basename to /sdcard. */
            if (!args.kmodel_path.empty() &&
                access(args.kmodel_path.c_str(), R_OK) != 0)
            {
                const size_t slash = args.kmodel_path.find_last_of("/\\");
                const std::string basename =
                    slash == std::string::npos
                        ? args.kmodel_path
                        : args.kmodel_path.substr(slash + 1);
                const std::string sdcard_path = "/sdcard/" + basename;
                if (!basename.empty() &&
                    access(sdcard_path.c_str(), R_OK) == 0)
                {
                    std::cout << "Use SD-card model: " << sdcard_path
                              << std::endl;
                    args.kmodel_path = sdcard_path;
                }
            }
