---
slug: /
description: 从开发环境、V853 系统开发、LVGL 9 到 EdgeOS Desktop，使用 AI Agent 完成嵌入式 Linux UI 移植实战。
---
...
*课程从可复现环境出发，依次完成系统构建、板级验证和 UI 上板，再为 EdgeOS 与更多芯片平台的迁移建立基础；图中的桌面界面用于展示后续移植的产品形态。*
...
![开发者借助 AI Agent 将工程问题转化为桌面设置中心的验证成果，作为 V853 Tina 后续迁移参考](./images/why-learn-ai-agent-real-ui-v4.png)

*从工程问题、AI 协作到实机验证，右侧设置中心展示 EdgeOS Desktop 的目标产品形态。*
...
| [Part 3：EdgeOS V853 移植](/docs/edgeos-v853-porting) | EdgeOS 核心桌面、Settings 设置中心、页面导航与 Tina 软件包 | 得到兼容横屏与竖屏的 EdgeOS Desktop 核心 UI 和只读设置中心 | 预览开放 |

:::tip 推荐学习顺序

先完成 Part 0 的统一环境，再进入 Tina SDK 编译和板级验证；确认 LCD 与触摸硬件正常后，完成 LVGL 9 移植，最后把 EdgeOS Desktop 核心 UI 接到已经验证的显示与输入链路。这样遇到问题时，更容易判断问题来自环境、硬件、Linux 驱动还是应用层。
...
- 一个面向 V853 多种屏幕形态的 EdgeOS Desktop 核心 UI、Settings 设置中心移植样例与真机验收方法。
- 一套以权限边界、证据和验收清单为核心的 AI Agent 协作方法。

## EdgeOS Desktop 真机界面预览

下面的截图展示 EdgeOS Desktop 把系统设置、设备能力与 AI 应用组织成统一桌面体验后的产品形态。

:::note 截图与课程阶段

这些界面用作 EdgeOS V853/Tina 移植的目标体验参考。Part 3 当前开放核心桌面与 Settings 设置中心适配，不代表 Camera、NPU、OTA 等完整功能已经在 V853 上实现。
...
| 阶段 | 主题 | 状态 |
| --- | --- | :---: |
| Part 3 | EdgeOS V853 移植 | 核心桌面与 Settings 设置中心预览开放 |
| Part 4 | V851s、V821、V861 跨平台验证 | 规划中 |
| Part 5 | Camera、MPP、NPU 与性能专题 | 规划中 |

> Part 3 后续章节会按实机验证进度逐步补充；当前导航展示已经提供正文的核心桌面与 Settings 设置中心章节。
